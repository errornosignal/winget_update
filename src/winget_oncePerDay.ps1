...
$logFile = "$env:UserProfile\OneDrive\Documents\Scripts\winget\winget-task_LastRunDate.txt"
...
if ($lastRunDate -ne $today) {
    ...
    Start-Process "cmd.exe" "/c `"$env:UserProfile\OneDrive\Documents\Scripts\winget\winget_upgrade.bat`"" -NoNewWindow
